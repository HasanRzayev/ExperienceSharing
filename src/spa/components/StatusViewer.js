...
const StatusViewer = ({ isOpen, onClose, statuses, currentUser, onStatusDelete, initialIndex = 0 }) => {
  ...
  // Update statusesList when statuses prop changes - ensure it's always an array
  useEffect(() => {
    if (Array.isArray(statuses)) {
      setStatusesList(statuses);
    } else {
      console.warn("StatusViewer: statuses prop is not an array:", statuses);
      setStatusesList([]);
    }
  }, [statuses]);

  // Ensure currentIndex is valid and get currentStatus safely
  const safeIndex = useMemo(() => Math.max(0, Math.min(currentIndex, statusesList.length - 1)), [currentIndex, statusesList.length]);
  const currentStatus = useMemo(() => {
    if (!Array.isArray(statusesList) || statusesList.length === 0) return null;
    return statusesList[safeIndex];
  }, [statusesList, safeIndex]);
  ...
  // Ensure statusesList is an array before accessing
  if (!Array.isArray(statusesList) || statusesList.length === 0) {
    console.warn("StatusViewer: statusesList is not an array or empty:", statusesList);
    return null;
  }
  
  // Ensure currentStatus exists (already calculated above)
  if (!currentStatus) {
    console.warn("StatusViewer: currentStatus is null/undefined");
    return null;
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black">
      <div className="relative flex size-full items-center">
        ...
        {currentIndex > 0 && (
          <button
            onClick={handlePrev}
            className="absolute left-4 z-50 rounded-full bg-black/30 p-4 text-white transition-all hover:bg-black/50"
          >
        ...
        {currentIndex < statusesList.length - 1 && (
          <button
            onClick={handleNext}
            className="absolute right-4 z-50 rounded-full bg-black/30 p-4 text-white transition-all hover:bg-black/50"
          >
        ...
        <div className="relative flex size-full items-center justify-center">
          ...
          <div className="absolute inset-x-4 top-4 z-40">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <h3 className="font-bold text-white">{currentStatus.user?.firstName} {currentStatus.user?.lastName}</h3>
                <p className="text-sm text-white/80">@{currentStatus.user?.userName}</p>
              </div>
              <div className="flex items-center gap-2">
                {isOwnStatus && (
                  <button
                    onClick={handleDelete}
                    className="cursor-pointer rounded-full p-2 text-red-400 transition-all hover:bg-red-500/20 hover:text-red-500"
                    title="Delete status"
                ...
                <button
                  onClick={onClose}
                  className="rounded-full p-2 text-white transition-all hover:bg-black/30 hover:text-gray-300"
                  title="Close"
                  style={{ minWidth: '40px', minHeight: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                >
          ...
          {currentStatus.text && (
            <div className="absolute inset-x-0 bottom-20 z-40 px-8">
              <p className="mx-auto max-w-2xl rounded-lg bg-black/50 p-4 text-center text-lg text-white shadow-2xl backdrop-blur-md">
                {currentStatus.text}
              </p>
            </div>
          )}
          ...
          <div className="absolute inset-x-4 bottom-4 z-40 flex items-center justify-between">
            ...
            <div className="flex gap-1">
              {Array.isArray(statusesList) && statusesList.length > 0 && statusesList.map((_, idx) => (
                <div
                  key={idx}
