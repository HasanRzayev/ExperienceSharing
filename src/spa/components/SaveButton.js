...
const SaveButton = ({ experienceId, renderAsMenuItem = false }) => {
  ...
  if (renderAsMenuItem) {
    return (
      <button
        onClick={handleSave}
        disabled={loading}
        className="flex w-full items-center gap-3 px-4 py-3 text-left text-gray-700 transition-colors hover:bg-gray-100 disabled:opacity-50 dark:text-gray-300 dark:hover:bg-gray-700"
      >
        <svg 
          className={`size-5 ${isSaved ? 'text-yellow-500' : 'text-gray-500 dark:text-gray-400'}`}
  ...
  return (
    <button
      onClick={handleSave}
      disabled={loading}
      className={`rounded-full border-2 bg-white bg-opacity-90 p-2.5 shadow-lg transition-all duration-200 hover:bg-opacity-100 hover:shadow-xl dark:bg-gray-800 ${
        isSaved 
          ? 'border-yellow-400 dark:border-yellow-500' 
          : 'border-gray-200 dark:border-gray-700'
      } z-10 hover:border-yellow-400 disabled:opacity-50 dark:hover:border-yellow-500`}
