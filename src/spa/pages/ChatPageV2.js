...
const ChatPageV2 = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (currentUser) {
        await Promise.all(usersList.map(async (contactUser) => {
          try {
            ...
            console.log(`[ChatPageV2] ✅ Badge set for user ${contactUser.id}: ${finalUnreadCount}`);
          } catch (err) {
      ...
      // All promises are already handled in the Promise.all above (line 617)
      // Now set the state with the collected counts
      console.log(`[ChatPageV2] 🔄 Final counts object before setting state:`, counts);
      console.log(`[ChatPageV2] 🔄 UnreadCounts state will be updated with:`, Object.keys(counts).length, 'users');
      
      // Log each user's unread count for debugging
      Object.keys(counts).forEach(userId => {
        console.log(`[ChatPageV2] User ${userId}: unreadCount = ${counts[userId]}`);
      });
      
      // IMPORTANT: Ensure state update triggers re-render by creating a new object
      // Also ensure keys are properly set (both string and number versions work)
      const newUnreadCounts = { ...counts };
      
      // Also set string versions of keys for compatibility
      Object.keys(counts).forEach(key => {
        const numKey = Number(key);
        if (!isNaN(numKey)) {
          newUnreadCounts[String(key)] = counts[key];
          newUnreadCounts[numKey] = counts[key];
        }
      });
      
      console.log(`[ChatPageV2] 🔄 New unreadCounts object for state:`, newUnreadCounts);
      console.log(`[ChatPageV2] 🔄 Checking user 2:`, {
        counts2: counts[2],
        newUnreadCounts2: newUnreadCounts[2],
        newUnreadCounts2String: newUnreadCounts['2']
      });
      
      setUnreadCounts(newUnreadCounts);
      setLastMessageTimes(lastTimes);
      
      console.log(`[ChatPageV2] ✅ UnreadCounts state updated - Total users: ${Object.keys(newUnreadCounts).length}`);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-100 py-8 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      <div className="mx-auto max-w-7xl px-4">
        <div className="overflow-hidden rounded-2xl bg-white shadow-lg dark:bg-gray-800" style={{ height: '85vh' }}>
          <div className="flex h-full">
            {/* Sidebar */}
            <div className="flex w-1/3 flex-col border-r border-gray-200 dark:border-gray-700">
              {/* Header with Tabs */}
              <div className="border-b border-gray-200 bg-gradient-to-r from-purple-600 to-blue-600 p-4 dark:border-gray-700">
                <div className="mb-4 flex items-center justify-between">
                  <h2 className="text-xl font-bold text-white">Messages</h2>
                  {activeTab === 'groups' && (
                    <button
                      onClick={() => setShowCreateGroupModal(true)}
                      className="rounded-full bg-white p-2 text-purple-600 transition-colors hover:bg-purple-50"
                      title="Create Group"
                    >
                      <FaPlus />
                    </button>
                ...
                {activeTab === 'chats' ? (
                  users.length === 0 ? (
                    ...
                    [...users].sort((a, b) => {
                      ...
                      // IMPORTANT: Check both string and number keys for user.id
                      const userId = user.id;
                      const unreadCountNumber = unreadCounts[userId] ?? unreadCounts[String(userId)] ?? unreadCounts[Number(userId)] ?? 0;
                      const unreadCount = unreadCountNumber || 0;
                      
                      // Debug log for user 2 specifically
                      if (userId === 2 || userId === '2') {
                        console.log(`[ChatPageV2] 🎯 UI Render for user 2:`, {
                          userId,
                          userIdType: typeof userId,
                          userIdString: String(userId),
                          userIdNumber: Number(userId),
                          unreadCountsKeys: Object.keys(unreadCounts),
                          unreadCountsUserId: unreadCounts[userId],
                          unreadCountsUserIdString: unreadCounts[String(userId)],
                          unreadCountsUserIdNumber: unreadCounts[Number(userId)],
                          finalUnreadCount: unreadCount,
                          unreadCountsObject: unreadCounts
                        });
                      }
                      ...
                      <div
                        ...
                        className={`cursor-pointer border-b border-gray-100 p-4 transition-colors hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-700 ${
                          selectedChat?.id === user.id && chatType === 'user' ? 'border-l-4 border-l-purple-600 bg-purple-50 dark:bg-purple-900/20' : ''
                        ...
                        <div className="flex items-center gap-3">
                          <div className="relative shrink-0">
                            <img
                              ...
                              {unreadCount > 0 && (
                                <div className="absolute -right-1 -top-1 flex h-5 min-w-[20px] items-center justify-center rounded-full border-2 border-white bg-red-500 px-1.5 text-xs font-bold text-white dark:border-gray-800">
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center justify-between gap-2">
                                <h3 className={`truncate font-semibold ${unreadCount > 0 ? 'font-bold text-gray-900 dark:text-white' : 'text-gray-800 dark:text-white'}`}>
                                  {displayName}
                                </h3>
                              </div>
                  ...
                  groups.length === 0 ? (
                    ...
                    groups.map((group) => {
                      ...
                      <div
                        ...
                        className={`cursor-pointer border-b border-gray-100 p-4 transition-colors hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-700 ${
                          selectedChat?.id === group.id && chatType === 'group' ? 'border-l-4 border-l-purple-600 bg-purple-50 dark:bg-purple-900/20' : ''
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          <img
            ...
            <div className="flex flex-1 flex-col">
              {selectedChat ? (
                <>
                  ...
                  <div className="border-b border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
                    <div className="flex items-center justify-between">
                              ...
                              <>
                                ...
                                <div className="min-w-0 flex-1">
                                  ...
                                  <button
                                    ...
                                    className="cursor-pointer text-left text-xs text-purple-600 hover:underline dark:text-purple-400"
                                  >
                      ...
                      <div className="relative">
                        ...
                        {showChatOptions && (
                          <div className="absolute right-0 top-full z-50 mt-2 w-56 rounded-xl border border-gray-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-800">
                            {chatType === 'group' ? (
                              /* Group Chat Options */
                              <div className="py-2">
                                <button
                                ...
                                >
                                  ...
                                  <span className="font-medium text-red-600 dark:text-red-400">Leave Group</span>
                                </button>
                              </div>
                            ) : (
                              /* User Chat Options */
                              <div className="py-2">
                                ...
                                <button
                                  ...
                                  className="flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-green-50 dark:hover:bg-green-900/20"
                                >
                                  <FaCheck className="text-green-600" />
                                  <span className="font-medium text-green-600 dark:text-green-400">Unblock User</span>
                                </button>
                              </div>
                            )}
                          </div>
                            ...
                            <div className="flex max-w-[70%] items-start gap-2">
                              ...
                              <div>
                                ...
                                <div
                                  className={`rounded-2xl px-4 py-3 shadow-md ${
                                    isOwnMessage
                                      ...
                                      : 'border border-gray-200/60 bg-white/90 text-gray-800 backdrop-blur dark:border-gray-700/60 dark:bg-gray-800/90 dark:text-white'
                                  } max-w-[520px]`}
                                ...
                                >
                                  ...
                                  <div className={`mt-2 flex items-center gap-2 ${isOwnMessage ? 'text-purple-100' : 'text-gray-500'}`}>
                                    <span>
                  ...
                  {/* File Preview */}
                  {filePreview && (
                    <div className="border-t border-gray-200 bg-gray-100 p-4 dark:border-gray-700 dark:bg-gray-700">
                      <div className="flex items-center space-x-3 rounded-xl bg-white p-3 dark:bg-gray-800">
                        <div className="shrink-0">
                          {filePreview.type.startsWith('image/') ? (
                            ...
                            <div className="flex size-12 items-center justify-center rounded-lg bg-gray-600">
                              <span className="text-xl text-white">📄</span>
                            </div>
                          )}
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="truncate font-medium text-gray-800 dark:text-white">{filePreview.name}</p>
                          <p className="text-sm text-gray-500 dark:text-gray-400">
                  ...
                  <form onSubmit={handleSendMessage} className="border-t border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="relative">
                        <button
                          type="button"
                          className={`rounded-full p-3 transition-colors hover:bg-gray-100 dark:hover:bg-gray-700 ${
                            file ? "bg-green-100 dark:bg-green-900" : ""
                          }`}
                          onClick={() => setDropdownOpen(!isDropdownOpen)}
                          title={file ? "File selected" : "Add media"}
                        >
                          <FaPlus className={`size-5 ${file ? "text-green-600" : "text-gray-600 dark:text-gray-400"}`} />
                        </button>

                        {isDropdownOpen && (
                          <div className="absolute bottom-full left-0 z-10 mb-2 w-48 rounded-xl border border-gray-200 bg-white p-2 shadow-lg dark:border-gray-600 dark:bg-gray-700">
                            <button
                              type="button"
                              className="flex w-full items-center space-x-3 rounded-lg p-3 text-gray-700 transition-colors hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-600"
                      ...
                      <input
                        ...
                        className="flex-1 rounded-full border border-gray-300 px-4 py-2 focus:border-transparent focus:ring-2 focus:ring-purple-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                      />
                      ...
                      {showPicker && (
                        <div className="absolute bottom-20 right-20 z-30">
                          <div className="rounded-xl border border-gray-200 bg-white p-2 shadow-2xl dark:border-gray-600 dark:bg-gray-800">
                            <div className="mb-2 flex items-center justify-between px-2">
                              <h4 className="text-sm font-semibold text-gray-800 dark:text-white">Choose Emoji</h4>
                              <button
                                type="button"
                      ...
                      {/* GIF Picker Modal */}
                      {showGifPicker && (
                        <div className="absolute bottom-20 right-32 z-30 max-h-80 w-72 overflow-auto rounded-xl border border-gray-200 bg-white p-4 shadow-2xl dark:border-gray-600 dark:bg-gray-800">
                          <div className="mb-3 flex items-center justify-between">
                            <h4 className="font-semibold text-gray-800 dark:text-white">Choose a GIF</h4>
                            <button
                              type="button"
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {gifs.map((gif) => (
                              <img
                                ...
                                alt="GIF"
                                className="h-auto w-full cursor-pointer rounded-lg transition-opacity hover:opacity-75"
                      ...
                      <button
                        ...
                        className="rounded-full bg-gradient-to-r from-purple-600 to-blue-600 px-6 py-2 font-medium text-white transition-all hover:from-purple-700 hover:to-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        <FaPaperPlane />
                      </button>
                    </div>
                ...
                <div className="flex flex-1 items-center justify-center bg-gray-50 dark:bg-gray-900">
                  <div className="text-center text-gray-400">
                    {activeTab === 'chats' ? (
                      <>
                        <FaUser className="mx-auto mb-4 text-6xl" />
                        <p className="text-lg">Select a contact to start chatting</p>
                      </>
                    ) : (
                      <>
                        <FaUsers className="mx-auto mb-4 text-6xl" />
      ...
      {showCreateGroupModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="max-h-[80vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white dark:bg-gray-800">
            ...
            <form onSubmit={handleCreateGroup} className="space-y-6 p-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full resize-none rounded-lg border border-gray-300 px-4 py-2 focus:border-transparent focus:ring-2 focus:ring-purple-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                />
              </div>
              ...
              <div>
                ...
                <div className="relative mb-3">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-gray-300 py-2 pl-10 pr-4 focus:border-transparent focus:ring-2 focus:ring-purple-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                  />
                </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  type="submit"
                  className="flex-1 rounded-lg bg-gradient-to-r from-purple-600 to-blue-600 px-4 py-2 font-medium text-white transition-all hover:from-purple-700 hover:to-blue-700"
                >
                  Create Group
                </button>
              </div>
      ...
      {showMembersModal && chatType === 'group' && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 p-4" onClick={() => setShowMembersModal(false)}>
          <div className="max-h-[80vh] w-full max-w-md overflow-y-auto rounded-2xl bg-white shadow-2xl dark:bg-gray-800" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="space-y-3">
                  {groupMembers.map((member, index) => {
                    ...
                    return (
                      <div
                        key={member.id || index}
                        className="flex items-center gap-3 rounded-lg bg-gray-50 p-3 transition-colors hover:bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600"
                      >
                        <img
                          src={profileImg || `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&background=random`}
                          alt={displayName}
                          className="size-12 shrink-0 rounded-full border-2 border-purple-500 object-cover"
                          onError={(e) => {
                            e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(username)}&background=667eea&color=fff`;
                          }}
                        />
                        <div className="min-w-0 flex-1">
                          <h3 className="truncate font-semibold text-gray-800 dark:text-white">
                            {fullName}
                            {isCurrentUser && <span className="ml-2 text-xs text-gray-500">(Siz)</span>}
                          </h3>
                          <p className="truncate text-sm text-gray-500 dark:text-gray-400">
                            @{username}
                          </p>
